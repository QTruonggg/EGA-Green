function egaFlashSale() {
	...
	function renderProgress(item) {
		...
		let useTag = window.falshSale.useTags
		...
		if (percent == 100 || !available) {
			...
			percent = 100 - percent
			percent = percent == 0 && available ? 0 : percent
		...
		if (!useSoldQuantity || useTag) soldNumber.text(quantityType == 'inventory' ? stock - soldQty : soldQty)
  }
	function updateQty() {
...
${html.join(`	<div class="ega-badge-ctd__colon" > : </div>`)}
	...
	if (distance > 0) {
		...
		if (!useSoldQuantity && ! window.falshSale.useTags) {
		let start = 0
			setInterval(function () {
				if(start){
				updateQty()
