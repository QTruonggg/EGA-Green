	...
	$(document).on('click', '.quick-view', function(e) {
		if($(window).width() > 1025){
			...
			Haravan.getProduct(producthandle,function(product) {
                ...
                console.log(featured_image,no_image_placeholder)
				...
				if(product.price < 1 && product.variants.length < 2){			
					...
					if(product.available){
						if (product.variants[0].available) {
                            ...
                            $('.soluong1').show();
					        	quickview.find(".add_to_cart_detail").text("Thêm vào giỏ hàng").removeClass("disabled").attr("disabled",false);	
                        }else{
					...
					if (product.variants[0].compare_at_price_max > product.variants[0].price) {
						
						quickview.find(".old-price").html(Haravan.formatMoney(product.variants[0].compare_at_price_max, window.money_format)).show();
		...
		function loadQuickViewSlider(n, r) {
            ...
            if(!n.featured_image){
                s = no_image_placeholder
            }
            console.log(n.featured_image,no_image_placeholder)
				...
				for (i in n.images) {
					...
					if(u.indexOf("video") > -1){
						...
						if(video_tags.length > 0){
							...
							var f = '<li><a href="javascript:void(0)"' + 'data-videocode="' + video_code + '" data-imageid="' + n.id + '"" data-zoom-image="' + u + '" title="title"><img src="' + u + '" alt="Office World" /></a></li>';
						}
					}else{
